...
const REASONS: Record<string,string> = {
  NO_BUY_SETUP:'giá và khối lượng chưa đủ điều kiện mua', MISSING_ACTION_MAP:'chưa có bản đồ hành động đủ điều kiện phát hành',
  UPSIDE_TOO_LOW:'dư địa tăng tại điểm vào hiện tại chưa đủ hấp dẫn', RR_BELOW_2:'lợi nhuận kỳ vọng chưa đạt gấp đôi khoản lỗ dự kiến',
...
export function buildResearchSnapshot(context: any): any {
  ...
  return {
    ...
    setup:{
      ...
      volume:firstNum(tech.volume,post.volume,a.volume), vol20:firstNum(tech.vol20,tech.volume20,post.vol20,post.volume20,a.vol20), max_down_volume10:facts.downMax,
...
export function researchSnapshotText(context: any): string {
  ...
  const states:string[]=[]; if(x.setup.candidate_setup)states.push(`điều kiện giá ${x.setup.candidate_setup}`); if(x.setup.new_position_state)states.push(`mua mới ${x.setup.new_position_state}`); if(x.setup.holding_state)states.push(`nắm giữ ${x.setup.holding_state}`); if(x.setup.stage)states.push(`giai đoạn giá ${x.setup.stage}`); if(states.length)lines.push(`- Trạng thái: ${states.join(' · ')}.`);
  const facts=readableResearchFacts(context);
  const technical:string[]=[facts.priceText, facts.volumeText, facts.earlyVolumeText].filter(Boolean);
  pushMetric(technical,'Giá trung bình 10 phiên',x.setup.ma10,'price'); pushMetric(technical,'Giá trung bình 50 phiên',x.setup.ma50,'price'); pushMetric(technical,'Giá trung bình 150 phiên',x.setup.ma150,'price'); pushMetric(technical,'Giá trung bình 200 phiên',x.setup.ma200,'price');
  if(technical.length)lines.push(`- Kỹ thuật chi tiết: ${technical.join(' · ')}.`);
  ...
  const fundBits:string[]=[]; pushMetric(fundBits,'Doanh thu so với cùng kỳ',x.fundamentals.revenue_growth_yoy_pct,'pct'); pushMetric(fundBits,'Lợi nhuận so với cùng kỳ',x.fundamentals.profit_growth_yoy_pct,'pct'); pushMetric(fundBits,'Lãi trên mỗi cổ phiếu so với cùng kỳ',x.fundamentals.eps_growth_yoy_pct,'pct'); pushMetric(fundBits,'ROE',x.fundamentals.roe_pct,'pct'); pushMetric(fundBits,'ROA',x.fundamentals.roa_pct,'pct'); pushMetric(fundBits,'Biên LN',x.fundamentals.net_margin_pct,'pct'); pushMetric(fundBits,'D/E',x.fundamentals.debt_to_equity,'multiple'); if(fundBits.length)lines.push(`- Cơ bản: ${fundBits.join(' · ')}.`);
  const valBits:string[]=[]; pushMetric(valBits,'P/E',x.valuation.pe,'multiple'); pushMetric(valBits,'Forward P/E',x.valuation.forward_pe,'multiple'); pushMetric(valBits,'P/B',x.valuation.pb,'multiple'); pushMetric(valBits,'PEG',x.valuation.peg,'multiple'); pushMetric(valBits,'EV/EBITDA',x.valuation.ev_ebitda,'multiple'); pushMetric(valBits,'Giá trị ước tính',x.valuation.fair_value,'price'); if(valBits.length)lines.push(`- Định giá: ${valBits.join(' · ')}.`);
  const plan:string[]=[]; const low=num(x.trade_plan.buy_zone_low),high=num(x.trade_plan.buy_zone_high); plan.push(low!=null||high!=null?`Vùng giá mua tham khảo ${low!=null?fmtPrice(low):'—'}–${high!=null?fmtPrice(high):'—'}`:'Vùng giá mua tham khảo chưa có'); if(num(x.trade_plan.stop_loss)!=null)plan.push(`Mức cắt lỗ ${fmtPrice(x.trade_plan.stop_loss)}${num(x.trade_plan.downside_to_stop_pct)!=null?` (${fmtPct(x.trade_plan.downside_to_stop_pct)})`:''}`); else plan.push('Chưa có mức cắt lỗ'); for(const [label,value,upside] of [['Giá tham khảo ngắn hạn',x.trade_plan.target_near,x.trade_plan.target_near_upside_pct],['Giá tham khảo 3–6 tháng',x.trade_plan.target_3_6m,x.trade_plan.target_3_6m_upside_pct],['Giá tham khảo 12 tháng',x.trade_plan.target_12m,x.trade_plan.target_12m_upside_pct]] as any[]){if(num(value)!=null)plan.push(`${label} ${fmtPrice(value)}${num(upside)!=null?` (${num(upside)>=0?'+':''}${fmtPct(upside)})`:''}`);} if(num(x.trade_plan.risk_reward_to_base)!=null)plan.push(`Lợi nhuận kỳ vọng / khoản lỗ dự kiến ${fmtNumber(x.trade_plan.risk_reward_to_base,2)}`); if(num(x.trade_plan.position_initial_pct)!=null)plan.push(`tỷ trọng khởi đầu ${fmtPct(x.trade_plan.position_initial_pct)}`); lines.push(`- Kế hoạch nghiên cứu: ${plan.join(' · ')}.`);
  const riskBits:string[]=[]; pushMetric(riskBits,'ATR20',x.risk.atr20_pct,'pct',2); pushMetric(riskBits,'biến động 20 phiên',x.risk.realized_vol20_pct,'pct',2); pushMetric(riskBits,'Mức giảm lớn nhất từ đỉnh trong 60 phiên',x.risk.max_drawdown60_pct,'pct',2); if(riskBits.length)lines.push(`- Biến động / rủi ro: ${riskBits.join(' · ')}.`);
  const marketBits:string[]=[]; if(x.market.market_regime)marketBits.push(`thị trường ${x.market.market_regime}`); if(x.market.sector_regime)marketBits.push(`ngành ${x.market.sector_regime}`); if(marketBits.length)lines.push(`- Thị trường và ngành: ${marketBits.join(' · ')}.`);
  const supplyBits:string[]=[]; pushMetric(supplyBits,'Ước tính tỷ lệ cổ phiếu tự do chuyển nhượng',x.supply_institutional.free_float_proxy_pct,'pct'); pushMetric(supplyBits,'Tỷ lệ giao dịch so với cổ phiếu lưu hành trong 20 phiên',x.supply_institutional.float_turnover20_pct,'pct',2); if(supplyBits.length)lines.push(`- Cung / tổ chức: ${supplyBits.join(' · ')}.`);
  const catalystBits:string[]=[]; if(num(x.catalyst.official_items_30d)!=null)catalystBits.push(`${fmtNumber(x.catalyst.official_items_30d,0)} tin HOSE/30 ngày`); if(num(x.catalyst.official_items_90d)!=null)catalystBits.push(`${fmtNumber(x.catalyst.official_items_90d,0)} tin/90 ngày`); if(x.catalyst.latest_title)catalystBits.push(`mới nhất: ${x.catalyst.latest_title}${x.catalyst.latest_time?` (${x.catalyst.latest_time})`:''}`); if(catalystBits.length)lines.push(`- Tin doanh nghiệp chính thức: ${catalystBits.join(' · ')}.`);
