...
function appendPosition(answer,scope,ticker,watch){
  if(scope==="ticker"){
    ...
    if(x.cost_basis!=null)bits.push(`giá vốn tự khai báo ${x.cost_basis.toLocaleString("vi-VN")}đ`);
    if(x.portfolio_weight_pct!=null)bits.push(`tỷ trọng tự khai báo ${x.portfolio_weight_pct.toLocaleString("vi-VN",{maximumFractionDigits:1})}%`);
...
async function audit(client,userId,ticker,horizon,outcome,reason,httpStatus,started,remaining=null){
  try{ await client.rpc("record_stockradar_api_request_event",{p_user_id:userId,p_ticker:ticker,p_horizon:horizon,p_outcome:outcome,p_reason:reason,p_http_status:httpStatus,p_latency_ms:Math.max(0,Math.round(performance.now()-started)),p_rate_limit_remaining:remaining}); }
...
Deno.serve(async req=>{
  ...
  const {data:ud,error:ue}=await authClient.auth.getUser(token), user=ud?.user;
  if(ue||!user)return json({status:"UNAUTHORIZED"},401,origin);
  ...
  if(!["auto","ticker","portfolio"].includes(scopeRaw))return json({status:"INVALID_REQUEST",reason:"INVALID_SCOPE"},400,origin);
  const scope=scopeRaw==="portfolio"?"portfolio":scopeRaw==="ticker"||ticker?"ticker":"portfolio";
  if(scope==="ticker"&&!validTicker(ticker))return json({status:"INVALID_REQUEST",reason:"INVALID_TICKER"},400,origin);
  ...
  const watch=(Array.isArray(rows)?rows:[]).flatMap(r=>{
    const t=String(r.ticker||"").trim().toUpperCase(),h=String(r.horizon||"SHORT_TERM").trim().toUpperCase();
    if(!validTicker(t)||!validHorizon(h))return[];
    const own=r.owns_stock===true;
    return[{ticker:t,horizon:h,owns_stock:own,alert_enabled:PREMIUM.has(tier)&&r.alert_enabled===true,cost_basis:own?pos(r.cost_basis,.0001):null,portfolio_weight_pct:own?pos(r.portfolio_weight_pct,0,100):null}];
  });
  ...
  const [reports,research]=await Promise.all([
    ...
    Promise.all(researchTickers.map(async t=>{const {data,error}=await service.rpc("fetch_stockradar_ai_context",{p_ticker:t}); return{ticker:t,data,error};})),
  ]);
  const ready=reports.filter(r=>!r.error&&r.data?.status==="READY"), researchContexts=research.flatMap(r=>{if(r.error)return[]; const normalized=normalizeResearchContext(r.data); return normalized?[normalized]:[];});
  const researchReady=researchContexts.some(c=>c.context_grade==="RESEARCH_READY"), referenceReady=researchContexts.length>0, mode=stockRadarMode(ready.length>0,researchReady,referenceReady);

  const {data:qraw,error:qe}=await service.rpc("consume_stockradar_api_quota",{p_user_id:user.id,p_bucket:"stock_ai"}), q=qraw||{};
  if(qe||!qraw){await audit(service,user.id,scope==="ticker"?ticker:"",horizon,"SERVICE_UNAVAILABLE","AI_QUOTA_RPC_FAILED",503,started);return json({status:"SERVICE_UNAVAILABLE",reason:"AI_QUOTA_RPC_FAILED"},503,origin);}
  const remN=Number(q.remaining),remaining=Number.isFinite(remN)?remN:null,limitN=Number(q.limit),retry=Number(q.retry_after||0),reset=String(q.reset_at||"")||null,rate={};
  if(Number.isFinite(limitN))rate["X-RateLimit-Limit"]=String(limitN);
  if(remaining!=null)rate["X-RateLimit-Remaining"]=String(remaining);
  if(q.allowed!==true){
    if(retry>0)rate["Retry-After"]=String(retry);
    await audit(service,user.id,scope==="ticker"?ticker:"",horizon,"RATE_LIMITED","AI_RATE_LIMITED",429,started,remaining);
    return json({status:"RATE_LIMITED",reason:"AI_RATE_LIMITED",answer:tier==="FREE"?"Bạn đã dùng đủ 10 lượt StockRadar AI hôm nay. Hạn mức Free được làm mới lúc 00:00 giờ Việt Nam.":"Hạn mức StockRadar AI hiện tại đã dùng hết. Hãy thử lại sau khi hạn mức được làm mới.",tier,quota:{remaining:0,limit:Number.isFinite(limitN)?limitN:null,reset_at:reset,reset_timezone:q.daily_reset_timezone||null},retry_after:retry},429,origin,rate);
  }

  const action=ready.map(r=>normReport(r.data)), researchFallback=scope==="ticker"?(researchContexts[0]||null):researchContexts;
  let fallback=deterministicStockRadarAnswer({mode,researchContext:researchFallback,actionContext:action,question:message});
  ...
  const ids=[...new Set([...action.map(x=>String(x.snapshot_id||"")),...researchContexts.map(x=>String(x.snapshot_id||""))].filter(Boolean))];
  const source={action_gate:ready.length?"READY":"PENDING",research_ready:researchReady,reference_ready:referenceReady,context_grades:{research_ready:researchContexts.filter(c=>c.context_grade==="RESEARCH_READY").length,reference_only:researchContexts.filter(c=>c.context_grade!=="RESEARCH_READY").length,requested:researchTickers.length},snapshot_id:ids.length===1?ids[0]:null,snapshot_count:ids.length,generated_at:latest([...action.map(x=>x.generated_at),...researchContexts.map(x=>x.generated_at)]),as_of_date:latest(researchContexts.map(x=>x.as_of_date)),ready_horizons:scope==="ticker"?ready.map(x=>x.horizon):undefined,ready_reports:ready.length,covered_tickers:scope==="portfolio"?[...new Set(researchContexts.map(x=>String(x.ticker||"")).filter(Boolean))]:undefined};
  const quota={remaining,limit:Number.isFinite(limitN)?limitN:null,unlimited:q.unlimited===true,reset_at:reset,reset_timezone:q.daily_reset_timezone||null};

  if(mode==="METHOD_ONLY"){
    await audit(service,user.id,scope==="ticker"?ticker:"",horizon,"AI_READY","STOCKRADAR_CORE_METHOD_ONLY",200,started,remaining);
    return json({status:"READY",scope,ticker:scope==="ticker"?ticker:null,horizon,tier,mode,answer_engine:"STOCKRADAR_CORE",answer:fallback,personalization,source,quota,quota_consumed:true},200,origin,rate);
  }
  const key=Deno.env.get("OPENAI_API_KEY")?.trim();
  if(!key||Date.now()<providerDisabledUntil){
    const reason=!key?"OPENAI_KEY_MISSING":"OPENAI_CIRCUIT_OPEN";
    await audit(service,user.id,scope==="ticker"?ticker:"",horizon,"AI_READY_FALLBACK",reason,200,started,remaining);
    return json({status:"READY_FALLBACK",reason,scope,ticker:scope==="ticker"?ticker:null,horizon,tier,mode,answer_engine:"STOCKRADAR_CORE",answer:fallback,personalization,source,quota,quota_consumed:true},200,origin,rate);
  }
  ...
  let response;
  try{
    response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/json"},body:JSON.stringify({model,instructions:STOCKRADAR_SYSTEM_CORE,input:JSON.stringify(context),max_output_tokens:scope==="portfolio"?1200:1000,store:false})});
  }catch{
    await audit(service,user.id,scope==="ticker"?ticker:"",horizon,"AI_READY_FALLBACK","OPENAI_NETWORK_ERROR",200,started,remaining);
    return json({status:"READY_FALLBACK",reason:"OPENAI_NETWORK_ERROR",scope,ticker:scope==="ticker"?ticker:null,horizon,tier,mode,answer_engine:"STOCKRADAR_CORE",answer:fallback,personalization,source,quota,quota_consumed:true},200,origin,rate);
  }
  ...
  if(!response.ok){
    ...
    await audit(service,user.id,scope==="ticker"?ticker:"",horizon,"AI_READY_FALLBACK",`OPENAI_${response.status}_${code}`,200,started,remaining);
    return json({status:"READY_FALLBACK",reason:`OPENAI_${response.status}_${code}`,scope,ticker:scope==="ticker"?ticker:null,horizon,tier,mode,answer_engine:"STOCKRADAR_CORE",answer:fallback,personalization,source,quota,quota_consumed:true},200,origin,rate);
  }
  const answer=appendPosition(openAIText(payload)||fallback,scope,ticker,watch);
  await audit(service,user.id,scope==="ticker"?ticker:"",horizon,"AI_READY",scope==="portfolio"?"MODEL_PLUS_STOCKRADAR_CORE_PORTFOLIO":"MODEL_PLUS_STOCKRADAR_CORE",200,started,remaining);
