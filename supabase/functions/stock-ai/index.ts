...
Deno.serve(async req=>{
  ...
  const base={project_bridge:projectBridgeMeta(projectContext,body.thread_id),...projectKnowledgeMeta(projectKnowledge),scope,ticker:scope==='ticker'?ticker:null,horizon,tier,mode,source,quota,quota_consumed:true,research_data:researchData,analysis:contexts.map(c=>analysisContract(c,action,horizon)),decision_cards:buildDecisionCards(contexts.length?contexts:scope==='ticker'?[{ticker}]:[],action,horizon,message)};
  ...
  return json({status:modelText?"READY":"READY_FALLBACK",...base,project_bridge:projectBridgeMeta(projectContext,body.thread_id,Boolean(modelText)),...projectKnowledgeMeta(projectKnowledge,Boolean(modelText)),answer_engine:modelText?"MODEL_PLUS_STOCKRADAR_CORE":"STOCKRADAR_CORE",answer},200,origin,rate);
