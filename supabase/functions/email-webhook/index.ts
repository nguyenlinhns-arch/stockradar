...
async function rpc(supabaseUrl: string, admin: AdminCredential, name: string, body: Record<string, unknown> = {}) {
...
async function webhookSecret(supabaseUrl: string, admin: AdminCredential): Promise<string> {
  const envSecret = String(Deno.env.get("RESEND_WEBHOOK_SECRET") || "").trim();
  if (envSecret.startsWith("whsec_")) return envSecret;
  try {
    const value = await rpc(supabaseUrl, admin, "get_stockradar_resend_webhook_secret_v1");
...
async function recordEvent(supabaseUrl: string, admin: AdminCredential, body: Record<string, unknown>) {
  return await rpc(supabaseUrl, admin, "record_stockradar_email_delivery_event_v1", body);
}
...
Deno.serve(async (req: Request) => {
  ...
  const secret = await webhookSecret(supabaseUrl, admin);
  if (!secret) {
    return new Response(JSON.stringify({ ok: false, reason: "WEBHOOK_SECRET_MISSING" }), { status: 503, headers: { "content-type": "application/json" } });
  }
