-- Align StockRadar product-email entitlement with the current Free/Premium contract.
-- Free accounts receive transactional account email only.
-- Daily briefs and product/action-alert email require Trial/Paid.
...
as $$
begin
  if new.account_status <> 'ACTIVE' or new.account_tier not in ('TRIAL','PAID') then
...
select
  ...
  (
    ...
    and (
      pref.daily_brief
      or pref.event_alerts
      or pref.post_session_digest
      or pref.weekly_report
    )
  ) as eligible_to_send,
  case
...
-- Fail closed any currently enabled non-Premium product-email preference.
update public.product_email_preferences pref
   set enabled = false,
       updated_at = now()
 where pref.enabled is true
   and exists (
     ...
     where prof.id = pref.user_id
       and prof.account_tier not in ('TRIAL','PAID')
   );