create table if not exists private.stock_signal_state (
  ...
  lane text not null check (lane in ('NEW_POSITION','HOLDING')),
  current_state text not null check (current_state in ('WAIT','BUY','HOLD','ADD','REDUCE','SELL')),
  setup text,
  ...
  primary key (ticker,horizon,lane)
);

create table if not exists private.stock_signal_events (
  id uuid primary key default gen_random_uuid(),
  event_key text not null unique,
  ticker text not null check (ticker ~ '^[A-Z]{3}$'),
  horizon text not null check (horizon in ('SHORT_TERM','MEDIUM_TERM','LONG_TERM','ACCUMULATION')),
  lane text not null check (lane in ('NEW_POSITION','HOLDING')),
  previous_state text not null check (previous_state in ('WAIT','BUY','HOLD','ADD','REDUCE','SELL')),
  current_state text not null check (current_state in ('WAIT','BUY','HOLD','ADD','REDUCE','SELL')),
  setup text,
  ...
  payload jsonb not null default '{}'::jsonb check (jsonb_typeof(payload) = 'object'),
  created_at timestamptz not null default now(),
  constraint stock_signal_events_material_change check (previous_state <> current_state)
);

create index if not exists stock_signal_events_ticker_time_idx
  on private.stock_signal_events (ticker, event_at desc);
create index if not exists stock_signal_events_actionable_time_idx
  on private.stock_signal_events (event_at desc) where actionable;

create table if not exists public.stockradar_notifications (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  event_id uuid not null references private.stock_signal_events(id) on delete cascade,
  ticker text not null check (ticker ~ '^[A-Z]{3}$'),
  horizon text not null check (horizon in ('SHORT_TERM','MEDIUM_TERM','LONG_TERM','ACCUMULATION')),
  lane text not null check (lane in ('NEW_POSITION','HOLDING')),
...
create index if not exists stockradar_notifications_user_time_idx
  on public.stockradar_notifications (user_id, created_at desc);
create index if not exists stockradar_notifications_user_unread_idx
  on public.stockradar_notifications (user_id, created_at desc) where read_at is null;

alter table public.stockradar_notifications enable row level security;

revoke all on table public.stockradar_notifications from public, anon, authenticated;
grant select on table public.stockradar_notifications to authenticated;

revoke all on table private.stock_signal_state from public, anon, authenticated;
revoke all on table private.stock_signal_events from public, anon, authenticated;

create policy stockradar_notifications_owner_select
...
begin
  ...
  loop
    ...
    loop
      ...
      v_title := case
        when v_next_state='BUY' and v_setup='POCKET_PIVOT' then 'ĐẠT ĐIỂM MUA – POCKET PIVOT'
        when v_next_state='BUY' and v_setup='EARLY_BREAKOUT' then 'ĐẠT ĐIỂM MUA – EARLY BREAKOUT'
        when v_next_state='BUY' and v_setup='CONFIRMED_BREAKOUT' then 'ĐẠT ĐIỂM MUA – XÁC NHẬN'
        when v_next_state='BUY' then 'ĐẠT ĐIỂM MUA'
        when v_next_state='ADD' then 'NHỒI LỆNH'
        ...
        if p_enqueue_emails
          ...
          v_email_payload := jsonb_build_object(
            ...
            'decision_card',jsonb_build_object(
              ...
              'next_review',coalesce(r.payload->'next_review',r.payload->'expected_horizon')
            ),
            'no_chase_notice',case when v_next_state='BUY' then 'Không mua đuổi ngoài Buy Zone; luôn kiểm tra trạng thái mới nhất trước khi đặt lệnh.' else null end,
...
create or replace function public.process_stockradar_alert_transitions_v1(
  p_emit_notifications boolean default true,
  p_enqueue_emails boolean default true
)
returns jsonb
...
begin
  ...
  return private.process_stockradar_alert_transitions_v1(p_emit_notifications,p_enqueue_emails);
end;
$$;

revoke all on function public.process_stockradar_alert_transitions_v1(boolean,boolean) from public, anon, authenticated;
grant execute on function public.process_stockradar_alert_transitions_v1(boolean,boolean) to service_role;

comment on table private.stock_signal_state is 'Canonical StockRadar per-ticker/per-horizon state memory. Never exposed to clients.';
comment on table private.stock_signal_events is 'Immutable material signal transitions derived only from approved production stock_report_cache rows.';
comment on table public.stockradar_notifications is 'Owner-only in-app notifications created from canonical actionable StockRadar state transitions.';
