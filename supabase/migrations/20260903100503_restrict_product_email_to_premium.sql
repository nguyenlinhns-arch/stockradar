-- Align product-email entitlement with the public Free/Premium product contract.
-- Free accounts may retain preference/interest rows but cannot enable product-email delivery.
...
-- Fail closed any previously enabled Free product-email preference.
update public.product_email_preferences pref
   set enabled = false,
       updated_at = now()
