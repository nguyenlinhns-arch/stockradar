...
| Minimum engine | PASS | Models, score/Coverage, anti-double-count, state machine, ranking, buy gate, volume and probability rules | 38 automated tests PASS | None | No production data adapter | Connect licensed provider | `engine/stockradar/*`, test output | engine source/tests |
...
| GitHub Pages deployment | PASS | Public repository, source, GitHub Actions build and Pages deployment completed | 38 tests PASS; workflow run 33500186454 attempt 2 SUCCESS; live site HTTP 200 | Initial run failed because Pages was not enabled; fixed and rerun successfully | Lead/event API remains intentionally disabled on static hosting | Keep Pages workflow green on every `main` update | `https://github.com/nguyenlinhns-arch/stockradar`, `https://nguyenlinhns-arch.github.io/stockradar/` | deploy workflow/source/status docs |
