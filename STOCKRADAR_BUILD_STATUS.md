...
| Website screenshot QA | TESTING | Static validation passed; live-browser QA planned after deployment | Local Playwright launch still cannot find Chromium | No layout failure observed yet | Local browser binary absent | Inspect deployed desktop/mobile pages and record result | `scripts/visual_qa.cjs`, `docs/QA_REPORT.md` | QA script/new responsive UI |
...
| GitHub Pages deployment | TESTING | Existing public deployment is healthy; Knowledge/home upgrade prepared locally | 41 tests and static build PASS locally | Initial historical run failed because Pages was not enabled; resolved | New commit not yet deployed; lead/event API intentionally disabled on static hosting | Push one commit, verify workflow and all 14 live routes | repository/live URL + next workflow | deploy workflow/source/status docs |
