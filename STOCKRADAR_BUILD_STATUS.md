...
| Website MVP local | PASS | Professional light finance-portal shell; data-status tape; dashboard Home; dense Radar workspace; four horizons; Knowledge hub + 6 guides; Radar/Trigger/Risk/Results/pricing/signup | 42 tests cover 14 routes, health, assets, metadata, sources, pricing, portal shell, truthful Radar state and static build | None | Public lead collection still requires a separate backend | Run live visual QA, then connect backend only after privacy/compliance gates | `website/*`, `docs/UX_BENCHMARK_VI.md`, web tests | HTML/CSS/JS/server/docs |
...
| Website screenshot QA | TESTING | Static structure and responsive rules for the new finance-portal interface are complete | 42 automated checks PASS; local Playwright launch attempted | None in automated checks | Local Chromium absent; new live deployment still needs desktop browser inspection and the 28-shot desktop/mobile matrix | Deploy, inspect live Home/Radar, then run `scripts/visual_qa.cjs` where Chromium is installed | `scripts/visual_qa.cjs`, `docs/QA_REPORT.md` | QA script/new portal UI |
