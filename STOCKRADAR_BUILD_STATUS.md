...
| Database production safety | PASS | Stock API gate remains disabled; no active manifest/snapshot; private stock report cache verified empty; private tables have no browser grants | Re-run on every production-gate change |
| Automated regression | TESTING | Strict HOSE ingestion, production data gate, report contract, cache binding/publisher, authenticated API/client, personalization, email and billing regressions are committed | Latest HEAD CI/deploy must complete green before marking current build PASS |
| Static GitHub Pages | TESTING | Fail-closed public build remains the expected surface while latest client/report-contract commits are under CI | Latest HEAD GitHub Actions/deploy confirmation |
...
Operational static interface, strict HOSE-only licensed-bundle ingestion, fail-closed market-data publication gate, Decision-Grade report contract, manifest-bound private cache, persistent account personalization, authenticated/rate-limited production API foundation, fail-closed email infrastructure and fail-closed billing foundation are implemented at their stated boundaries.
...
Critical path: `licensed public/derived-data rights → supplier HOSE bundle → fresh production manifest → model/Decision-Grade report batch → manifest-bound cache → reconciliation → activate exact manifest/snapshot only after compliance evidence → signed-in E2E/privacy drill → email/payment providers + webhooks → formal compliance → PRODUCTION_APPROVED`.
