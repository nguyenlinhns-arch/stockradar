...
- Product email: sending-only credential is stored in Vault and the worker health check confirms provider/sender configuration. Resend domain verification has completed and is recorded in the approval audit. The runtime's only remaining activation blocker is COMPLIANCE_APPROVAL_MISSING. No product email has been sent.
...
- DNS is configured at Mat Bao. Resend now reports the domain, DKIM TXT, return-path MX and SPF TXT all VERIFIED. The public status reads the audited sender result independently of delivery activation, so a verified domain does not imply mail is enabled. Never store a provider API token in this document or source control.
