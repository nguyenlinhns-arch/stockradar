...
class HomeWorkspaceV2Tests(unittest.TestCase):
    ...
    def test_workspace_data_renderer_is_fail_closed(self):
        ...
        for forbidden in ("Math.random", "mock", "demoTicker", "fakePrice"):
    ...
    def test_ai_opening_message_is_plain_language(self):
        js = self.read("website/assets/ai-center.js")
        self.assertIn("Tôi là StockRadar AI", js)
        self.assertIn("nếu dữ liệu chưa đủ chuẩn", js)
        for jargon in ("4M/Payback · CANSLIM", "SEPA/VCP", "VPA · Pocket Pivot"):
