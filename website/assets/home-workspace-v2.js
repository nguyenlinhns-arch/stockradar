(() => {
  ...
  function normalizeAiOpeningMessage() {
    const host = qs('[data-stockradar-ai-center]');
    const bubble = host?.querySelector('.sr-center-assistant .sr-center-bubble');
    if (!bubble || !String(bubble.textContent || '').startsWith('Tôi là StockRadar AI')) return;
    bubble.textContent = 'Tôi là StockRadar AI. Nhập một mã HOSE và hỏi thẳng điều bạn cần biết. Tôi sẽ kiểm tra dữ liệu doanh nghiệp, định giá, xu hướng, dòng tiền và rủi ro trước khi trả lời; nếu dữ liệu chưa đủ chuẩn, tôi sẽ nói rõ chưa đủ cơ sở để hành động.';
  }
  ...
  async function mount() {
    ...
    setTimeout(normalizeAiOpeningMessage, 0);
    setTimeout(normalizeAiOpeningMessage, 350);
    setTimeout(normalizeAiOpeningMessage, 1200);
