...
This is intentionally the last presentation layer. Earlier build transforms keep
legacy product contracts and regression coverage intact; this pass removes buyer-
facing explanations that are no longer useful in production and keeps only the
operational product surface: AI, signals, proof, pricing and account actions.

It never opens data, email, auth or billing gates and never fabricates market data.
"""
...
def inject_style(source: str) -> str:
    ...
    tag = f'<link rel="stylesheet" href="assets/{STYLE_NAME}?v=20260904-commercial1" {STYLE_MARKER}>\n'
    return source.replace("</head>", tag + "</head>", 1)


def remove_section(source: str, class_name: str, *, required: bool = True) -> str:
    pattern = re.compile(
        rf'\s*<section\b[^>]*class=["\'][^"\']*\b{re.escape(class_name)}\b[^"\']*["\'][^>]*>.*?</section>\s*',
...
def normalize_nav(source: str, current: str = "") -> str:
    ...
    nav = (
        '<nav class="nav-links" id="site-menu" aria-label="Điều hướng chính" data-nav-menu>'
        + link("./#stockradar-ai", "AI")
...
def normalize_header_register(source: str) -> str:
    source = re.sub(
        r'(<a\b[^>]*class=["\'][^"\']*\bheader-register-cta\b[^"\']*["\'][^>]*href=["\'])[^"\']+(["\'])',
        r'\1dang-ky/?plan=free\2',
        source,
        flags=re.IGNORECASE,
    )
    source = source.replace(">Đăng ký miễn phí</a>", ">Bắt đầu miễn phí</a>")
    return source
...
def commercial_home(source: str) -> str:
    ...
    source, count = re.subn(
        r'\s*<section class="home-section" aria-labelledby="proof-title">.*?</section>\s*',
        proof,
        source,
        count=1,
        flags=re.IGNORECASE | re.DOTALL,
    ...
    source = re.sub(
        r'\s*<form class="email-mini home-email-form".*?</form>\s*',
        "\n",
        source,
        count=1,
        flags=re.IGNORECASE | re.DOTALL,
    ...
    replacements = (
        (
            "Hỏi thẳng điều bạn cần biết: mua mới hay chờ, đang nắm giữ nên làm gì, vùng giá nào đáng chú ý và rủi ro nào có thể làm thay đổi quyết định.",
            "Mua hay chờ · Giữ hay bán · Vùng giá · Rủi ro.",
        ),
        ("Dữ liệu hành động mới nhất đã được StockRadar cho phép phát hành.", "Tín hiệu mới nhất."),
        (
            "Chỉ hiển thị khi dữ liệu đạt điều kiện phát hành. Không có mã đạt chuẩn cũng là một kết quả hợp lệ.",
            "Chỉ hiển thị tín hiệu đủ chuẩn.",
        ),
        ("Dùng AI trước. Nâng cấp khi cần nhiều hơn và cần theo dõi chủ động.", "Chọn mức sử dụng."),
        (
            "Khách có 3 câu/ngày. Free có 10 câu/ngày. Premium hỏi không giới hạn và có quyền nhận Action Alert khi hệ thống email production được kích hoạt.",
            "Khách 3 câu/ngày · Free 10 câu/ngày · Premium không giới hạn + Action Alert.",
        ),
        ("Dành cho người muốn tự hỏi AI, tra cứu mã và lưu danh sách theo dõi cơ bản.", "AI + tra cứu + watchlist cơ bản."),
        (
            "AI không giới hạn, lớp quyết định đầy đủ và quyền nhận Daily 09:00 / cảnh báo trong phiên sau khi email production đủ điều kiện vận hành.",
            "AI không giới hạn · quyết định đầy đủ · cảnh báo theo quyền gói.",
        ),
    )
...
def commercial_radar(source: str) -> str:
    ...
    source = source.replace("Radar toàn bộ cổ phiếu HOSE", "Radar HOSE")
    source = source.replace(
        "Quét full-universe → kiểm tra dữ liệu/thanh khoản → chấm điểm đa lớp → xếp hạng → Action Gate. Không đủ chuẩn thì không đưa vào danh sách hành động.",
        "Toàn HOSE. Chỉ hiển thị mã đủ điều kiện.",
    )
    source = re.sub(
        r'<a\b[^>]*href=["\']#(?:cach-dung-radar|phuong-phap)["\'][^>]*>.*?</a>',
        "",
        source,
        flags=re.IGNORECASE | re.DOTALL,
    )
    source = source.replace("FULL HOSE · GATED RADAR", "TOÀN HOSE")
    source = source.replace("NGẮN HẠN · 5–20 PHIÊN", "RADAR HOSE")
    source = source.replace("Radar theo trạng thái hành động", "Cơ hội theo trạng thái")
    source = source.replace("Dữ liệu · thanh khoản · bối cảnh · rủi ro", "Đủ điều kiện")
    source = source.replace(
...
def commercial_recommendations(source: str) -> str:
    ...
    source = remove_conversion_rail(source)
    source = re.sub(
        r'<div class="home-recommendation-status">.*?</div>\s*<section class="recommendation-reference-list"',
        '<div class="commercial-reco-summary"><div><span>Tín hiệu hiện tại</span><strong data-current-action-count>0 mã</strong></div><div><span>Phạm vi</span><strong>Toàn HOSE</strong></div></div><section class="recommendation-reference-list"',
        source,
        count=1,
        flags=re.IGNORECASE | re.DOTALL,
    )
    source = re.sub(
        r'\s*<p class="recommendation-reference-note">.*?</p>\s*',
        "\n",
        source,
        count=1,
        flags=re.IGNORECASE | re.DOTALL,
    ...
    source = source.replace(
        "StockRadar quét toàn HOSE nhưng không ép đủ số lượng. Chỉ mã vượt qua dữ liệu, thanh khoản, bối cảnh, chất lượng điểm vào và quản trị rủi ro mới được chuyển từ Radar sang tín hiệu hành động.",
        "Tín hiệu hành động đã được StockRadar phát hành.",
    )
...
def commercial_performance(source: str) -> str:
    ...
    source = remove_conversion_rail(source)
    source = source.replace(
        "Kích hoạt · Entry · Target/Stop · Benchmark. Người mua phải kiểm chứng được cả kết quả tốt lẫn kết quả xấu.",
        "Lịch sử tín hiệu, P/L và benchmark.",
    )
    ...
    source = source.replace('<section class="performance-workspace">', '<section class="performance-workspace">' + audit, 1)
    return source
...
def commercial_plans(source: str) -> str:
    ...
    source = re.sub(
        r'\s*<section class="plan-comparison" aria-labelledby="premium-notify-title">.*?</section>\s*',
        "\n",
        source,
        count=1,
        flags=re.IGNORECASE | re.DOTALL,
    )
    source = source.replace("199K/tháng", "199K/30 ngày")
    source = source.replace("199K / THÁNG", "199K / 30 NGÀY")
    source = source.replace("199.000đ / tháng*", "199.000đ / 30 ngày")
    source = source.replace("199.000đ / tháng", "199.000đ / 30 ngày")
    source = source.replace("0đ / tháng", "0đ")
    source = source.replace("Chọn Free hoặc đăng ký Premium", "Chọn gói StockRadar")
    source = re.sub(
        r'<p>Premium: tạo tài khoản → thanh toán 199\.000đ/30 ngày\. Không cần tạo Free rồi mới nâng cấp\.</p>',
        "",
        source,
        count=1,
    )
    source = source.replace("Miễn phí và 199K/30 ngày khác nhau thế nào?", "So sánh Free và Premium")
    source = source.replace(
    ...
    source = source.replace("SO SÁNH HAI GÓI", "SO SÁNH")
    source = source.replace("Radar 30 và theo ngành", "Radar và theo ngành")
    return source
...
def process_page(output: Path, route: str) -> None:
    ...
    source = read(page)
    source = inject_style(source)
    source = normalize_nav(source, route)
    source = normalize_header_register(source)
    source = normalize_footer(source)
...
def verify(output: Path) -> None:
    ...
    for label, (source, markers) in forbidden.items():
        for marker in markers:
            if marker in source:
                raise RuntimeError(f"Commercial {label} still contains verbose marker: {marker}")
    ...
    print("Commercial surface v1: PASS (AI → signals → proof → pricing; verbose buyer explanations removed)")
