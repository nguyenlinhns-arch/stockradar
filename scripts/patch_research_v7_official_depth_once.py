...
TARGET = Path('.github/workflows/research-decision-v7.yml')
...
if 'Download latest deep official HOSE disclosure artifact when available' not in text:
    if anchor not in text:
        raise SystemExit('V7 authoritative CA anchor missing; refusing broad patch')
...
if old_find in text:
    ...
    raise SystemExit('Catalyst V3 official-source locator anchor missing; refusing broad patch')
...
if 'scripts/merge_hose_official_disclosures_v1.py' not in text:
    if old_paths not in text:
        raise SystemExit('V7 trigger path anchor missing; refusing broad patch')
    text = text.replace(old_paths, new_paths, 1)

TARGET.write_text(text, encoding='utf-8')
print('patched research-decision-v7 for merged realtime + deep HOSE official disclosures')
